...
public class ImportInventarioFromXLSX {
    ...
    public ImportInventarioFromXLSX() {
...
//                             "./out_ImportProductoRelatedEntitiesFromXLSX_"+(System.currentTimeMillis())+".sql"};
        ...
        ImportInventarioFromXLSX ipreX = new ImportInventarioFromXLSX();
        try {
    ...
    private void importFromProductosXLSX(String fileName, int firstRow2Import, int lastRow2Import, String outputFile)
        ...
        try {
            ...
            for(int numSheet =0;iteratorXSSFSheet.hasNext();numSheet++) {
    ...
    private void processRowInventario(Row row) throws ParseException {
        int cells             = row.getPhysicalNumberOfCells();
        
        String cantidad       = row.getCell(0).getCellType()==XSSFCell.CELL_TYPE_FORMULA? String.valueOf(row.getCell(0).getCachedFormulaResultType()): row.getCell(0).toString();
        String clave          = row.getCell(1).toString();
        String codigoBarras   = row.getCell(2).toString();
        
        codigoBarras = codigoBarras.replace(".", "");
        codigoBarras = codigoBarras.contains("E")?codigoBarras.split("E")[0]:codigoBarras;
        
        System.out.println("-->>>"+cantidad+", "+clave+", "+codigoBarras);
    }
